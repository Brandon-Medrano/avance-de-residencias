...
d3.json('php/data3.php', function (data) {
    ...
    data.forEach(function (d) {
        d.dd = dateFormat.parse(d.fechacorte);
    ...
    var vc = crossfilter(data);
    var all = vc.groupAll();

    // Dimension by year
    var yearlyDimension = vc.dimension(function (d) {
    ...
    var dateDimension = vc.dimension(function (d) {
        return d.dd;
    });

    // Dimension by month
    var moveMonths = vc.dimension(function (d) {
        return d.month;
    });
    ...
    var fluctuation = vc.dimension(function (d) {
        return Math.round((d.close - d.open) / d.open * 100);
    });
    var fluctuationGroup = fluctuation.group();

    // Summarize volume by quarter
    var quarter = vc.dimension(function (d) {
    ...
    // Counts per weekday
    var dayOfWeek = vc.dimension(function (d) {
        var day = d.dd.getDay();
    ...
    nasdaqTable /* dc.dataTable('.dc-data-table', 'chartGroup') */
        ...
        .size(10)
        .columns([
            // Use the `d.fechacorte` field; capitalized automatically
            'fechacorte',
            // Use `d.open`, `d.close`
            'open',
            'close',
        ...
        .order(d3.ascending)        // (_optional_) custom renderlet to post-process chart using [D3](http://d3js.org)
